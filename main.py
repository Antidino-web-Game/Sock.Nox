...
def notify_users(message, exclude_pseudo=None):
    ...
    for user_addr, user_info in users.users.items():
        if user_info["pseudo"] != exclude_pseudo:
            send_message(user_info["socket"], message)
    ...
    try:
        while True:
            ...
            if message.startswith('/'):
                if message == '/pseudo':
                    send_message(client_socket, f"Les pseudos sont : {', '.join(users.get_all_pseudos())}")
                continue

            print(f"Reçu de {pseudo}({addr}): {message}")
            notify_users(f"[{pseudo}] : {message}", exclude_pseudo=pseudo)
    except Exception as e:
        print(f"Erreur avec {pseudo}({addr}) : {e}")

    print(f"Déconnexion de {pseudo}({addr})")
    notify_users(f"{pseudo} a quitté la discussion.", exclude_addr=addr)
